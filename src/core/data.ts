/* ===========================================================
   App Agency Tycoon — dados / balanceamento do jogo (F1 do PRD)
   Portado 1:1 de js/data.js — NENHUM número foi alterado.
   Módulo puro: sem DOM, sem Pixi (fronteira do core, lint).
   O shim window.DATA é montado em src/main.ts.
   =========================================================== */
...
export interface Tier {
  ...
  name: string;
  icon: string;
  /** mesas máximas neste tier */
  maxDesks: number;
  /** projetos simultâneos */
  slots: number;
  /** custo para expandir PARA este tier */
  cost: number;
  repReq: number;
}
...
export interface Cargo {
  ...
  name: string;
  emoji: string;
  /** pontos de trabalho por DIA de jogo */
  speed: number;
  ...
  repReq: number;
  quality: number;
  boost?: { quality?: number; rep?: number; team?: number };
  desc: string;
}
...
export interface Upgrade {
  ...
  emoji: string;
  cost: number;
  /** decorações aparecem na cena além do bônus */
  decor?: boolean;
  effect: { prodMult?: number; repMult?: number; contractFlow?: number; contractValue?: number };
  desc: string;
}
...
export interface FaseProjeto {
  id: 'design' | 'code' | 'tests';
  name: string;
  emoji: string;
  /** fração do trabalho onde a fase termina */
  ate: number;
  /** multiplicador de velocidade por cargo */
  weights: Record<CargoId, number>;
}

export interface Promocao {
  to: CargoId;
  /** xp em DIAS de jogo trabalhando */
  xp: number;
  /** taxa de promoção */
  cost: number;
}
...
export interface Rival {
  ...
  name: string;
  rep: number;
  /** crescimento de reputação por dia */
  growth: number;
}

/** Campos do estado usados pelas condições de conquista (subconjunto do save). */
export interface EstadoConquista {
  stats: { completed: number; earned: number; failed: number; hires?: number; promotions?: number };
  employees: unknown[];
  products?: unknown[];
  rivals?: { rep: number }[];
  rep: number;
...
export interface Conquista {
  ...
  cond: (s: EstadoConquista) => boolean;
}
...
export const CLIENTS: string[] = ['Padaria do Zé', 'PetShop Amigo', 'Studio Fitness', 'Loja da Ana', 'Clínica Vida',
...
export const PROJECT_TYPES: TipoProjeto[] = [
...
export const FIRST_NAMES: string[] = ['Ana', 'Bruno', 'Carla', 'Diego', 'Elisa', 'Felipe', 'Gabi', 'Hugo',
...
export const PHASES: FaseProjeto[] = [
...
export function phaseFor(frac: number): FaseProjeto {
  return PHASES.find((f) => frac < f.ate) || (PHASES[PHASES.length - 1] as FaseProjeto);
}

// ---- Evolução de carreira (trilha dev) ----
export const PROMOTIONS: Partial<Record<CargoId, Promocao>> = {
  junior: { to: 'pleno', xp: 8, cost: 900 },
  pleno: { to: 'senior', xp: 20, cost: 3300 },
};

// ---- Eventos aleatórios ----
export const EVENTS: EventoDef[] = [
...
export const PRODUCT_NAMES: string[] = ['TaskZen', 'FitTrack', 'NotaFácil', 'PetCare+', 'Receitou', 'Grana App',
...
// ---- Conquistas ----
export const ACHIEVEMENTS: Conquista[] = [
...
/** Pacote completo — mesmo formato do antigo window.DATA (shim em main.ts). */
export const DATA = {
  TIERS, ROLES, UPGRADES, CLIENTS, PROJECT_TYPES, xpForLevel,
